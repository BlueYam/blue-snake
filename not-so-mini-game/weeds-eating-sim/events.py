...
def vintage_bass_temptation(player):
    print("Ryo spots a vintage 1960s Fender Precision Bass in a shop window.")
    if player.money >= 5000:
        ...
        if choice == "y":
            ...
            player.cool += 30
            print(
                "Ryo is now broke, but she looks incredibly cool holding it. +30 COOL."
            )
        else:
        ...
        player.cool -= 5
        print(
            "Ryo presses her face against the glass. She can't afford it. The shopkeeper asked her to leave. -5 COOL."
        )
...
def kessoku_band_practice(player):
    ...
    if player.stamina > 30:
        ...
        player.cool += 10
        print(
            "Practice went great. Ryo's bass lines carried the song. -20 STAMINA, +10 COOL."
        )
    else:
        player.hp -= 10
        player.cool -= 10
        print(
            "Ryo passed out mid-song due to hunger. Nijika is disappointed. -10 HP, -10 COOL."
        )
...
def the_bocchi_effect(player):
    ...
    if choice == "1":
        ...
        print(
            "Ryo calmed her down. Nijika saw this and forgave 100 Yen of debt. -10 STAMINA."
        )
    else:
        player.cool += 15
...
def fan_encounter(player):
    ...
    if player.cool > 40:
        ...
        player.money += gift
        print(
            f"The fan bought Ryo a high-end curry! She pocketed the cash instead. +{gift} Yen."
        )
    else:
...
def nijika_intervention(player):
    if player.debt > 2000:
        print(
            "Nijika found Ryo's secret stash of expensive pedals while the debt remains unpaid."
        )
        player.money = 0
        player.debt -= 1000
        print(
            "Nijika confiscated all of Ryo's pocket money to pay off the debt. Money reset to 0, Debt -1000."
        )
    else:
